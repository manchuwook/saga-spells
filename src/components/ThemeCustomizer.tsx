...
export const ThemeCustomizer: React.FC = () => {
  ...
  return (
    <Paper p="md" withBorder shadow="sm" style={{ position: 'fixed', bottom: '20px', right: '20px', zIndex: 1000, maxWidth: opened ? '400px' : 'auto' }}>
      <Group justify="space-between" mb={opened ? "md" : 0}>
        <Button 
          onClick={() => setOpened(!opened)}
          variant="light"
        >
          {opened ? 'Hide Theme Settings' : 'Theme Settings'}
        </Button>
        {opened && (
          <Group>
            <Switch 
